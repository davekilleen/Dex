# Tasks

Your task backlog organized by priority.

## This Week

- [x] **Complete review of Ella's college surveys** — Due this morning #family ^task-20260403-001 ✅ 2026-04-07
- [ ] **Add current_status string field to IntelliQ state — each node overwrites it, Studio shows real-time progress as state updates stream** #wisory_product ^task-20260402-001
- [x] **Prepare for Experian hiring manager interview — research Ryan (hiring leader), Susan Allen, Michelle, housing unit strategy** — Interview completed Apr 2, advancing to next round #job_search ^task-20260318-001 ✅ 2026-04-02
- [x] **Share Wisory Score methodology document (data flows, transformations, sub-index calculations) with Sean for review** — From Digital Twin Strategy meeting #wisory_product ^task-20260323-001 ✅ 2026-03-23
- [x] **Build one-page Now/Near/Far roadmap for Intel IQ / Digital Sandbox — circulate to group** — Target EOD Mar 24. Follow-up meeting Thu/Fri #wisory_product ^task-20260323-002 ✅ 2026-03-24
- [x] **Complete Ella's Marshall College Survey for counselor recommendation letter** — Deadline Thursday Mar 26 #family ^task-20260324-001 ✅ 2026-03-24
- [x] **Update Dot Connector GTM frameworks 1-5 using team feedback, regenerate, and publish to shared Google Drive** — From GTM Strategy meeting #wisory_product ^task-20260324-002 ✅ 2026-03-28
- [x] **Arrange Jason Whitney review of GTM positioning and channel recommendations** — Once frameworks are ready #wisory_product ^task-20260324-003 ✅ 2026-03-28
- [x] **Combine Harriette's Shopify docs + Intel IQ material into Dot Connector for product/roadmap frameworks (v1)** — Share with Harriette when ready #wisory_product ^task-20260324-004 ✅ 2026-03-28
- [x] **Digest Digital Twin meeting transcript, recommend changes to Wisory Score overview, circulate to group** — Before Thu/Fri follow-up #wisory_product ^task-20260323-003 ✅ 2026-03-23
- [x] **Synthesize Leo 1:1 into written summary, email to Leo for verification, share with Adam and Dan** — From Regis and Leo 1:1 #wisory_product ^task-20260325-002 ✅ 2026-03-27
- [x] **Send Jason family members' names, ages, and current Cobra plan details for insurance quotes** — From Business Strategy meeting #wisory_product ^task-20260325-003 ✅ 2026-03-27
- [x] **Finish multi-year revenue projections (technology, staffing, sponsorship assumptions) and review with Jason on ambition framework and GTM plan** — From Business Strategy meeting #wisory_product ^task-20260325-004 ✅ 2026-03-28
- [ ] **After LangGraph training (next 2 days), initiate technical discussion with Terry about integrating LangGraph and tooling into product stack** — From Business Strategy meeting #wisory_product ^task-20260325-005
- [x] **Review Fred's comments on relevant files — determine if addressed in v2 docs, itemize revisions needed** — Don't create new doc versions #wisory_product ^task-20260325-006 ✅ 2026-03-28
- [x] **Polish first-principles breakdown (PE client challenges vs. Intel IQ + advisor) and send to Jason W and Jason Whitney** — From Regis and Jason 1:1 #wisory_product ^task-20260327-001 ✅ 2026-04-07
- [x] **Issue April consultant invoice to Jason at agreed monthly rate** — From Regis and Jason 1:1 #wisory_product ^task-20260327-002 ✅ 2026-03-31

<!-- Tasks promoted to this week's focus -->

## P0 - Urgent (max 3)

- [x] **Pick up scripts at Walgreens** #family ^task-20260405-001 ✅ 2026-04-06
- [x] **Buy apple juice and groceries at Super One** #family ^task-20260405-002 ✅ 2026-04-06
- [x] **Return call from Abby at Barr Engineering to schedule next Tuesday interview** #job_search ^task-20260328-003 ✅ 2026-03-31
- [x] **Drop off return at Dick's Sporting Goods** #family ^task-20260328-001 ✅ 2026-03-31
- [x] **Buy dog food** #family ^task-20260328-002 ✅ 2026-03-31

<!-- Critical items that must be done today/tomorrow -->

## P1 - Important (max 5)

- [x] **Send Mantera IntelliQ report to Wisory leadership team — incorporate Whitney's answers + press release, review at 10am CT tomorrow** — Due tonight/early Monday #wisory_product ^task-20260405-014 ✅ 2026-04-06
- [x] **Confirm Anne-Marie meeting** — Rescheduled by Anne-Marie, removed from cal #wisory_product ^task-20260405-003 ✅ 2026-04-05
- [x] **Review shares agreement from Jason — received, needs review and signature** #wisory_product ^task-20260405-004 ✅ 2026-04-07
- [x] **Send transcript and thoughts from Wes meeting to Jason** — Investor strategy debrief #wisory_product ^task-20260405-005 ✅ 2026-04-07
- [x] **Setup counseling appointment** — See email for details #family ^task-20260405-006 ✅ 2026-04-07
- [ ] **File provisional patent for Dot Connector AI — draft invention disclosure covering orchestration engine, contract enforcement, anti-truncation, schema validation, and adaptive learning loop** — See `04-Projects/Dot_Connector_AI_Patent.md` for full analysis and next steps #thought_leadership ^task-20260325-001
- [b] **Review Fog Line Advisors agreement and NDA and send feedback to Jim Black** — Feedback sent, waiting on Jim's response #job_search ^task-20260319-002
- [x] **Complete 2025 business and personal tax returns** — State + federal filed, quarterly estimates paid for both #family ^task-20260319-003 ✅ 2026-03-31
- [x] **Review PE sector activity report from James (Deerpath)** — Report received, needs review to recommend 3-4 target industries #wisory_product ^task-20260312-001 ✅ 2026-03-28
- [x] **Design phased engineering plan — v1.5 scope and incremental roadmap to full LangChain** — Extract industry/company/Wisory Score as v1.5, then build toward full implementation #wisory_product ^task-20260316-002 ✅ 2026-03-28

## P2 - Normal (max 10)

- [ ] **Compile all messaging framework feedback (mine + Fred's) into single updated pass, share with Maeve** — From 2026-04-13 Wisory Team Check-In #wisory_product ^task-20260413-001
- [ ] **Design short "brief" slide template for content creation based on agreed messaging framework** — From 2026-04-13 Wisory Team Check-In #wisory_product ^task-20260413-002
- [ ] **Run Intel IQ report for XTI Aerospace using Drone Nerds as anchor; identify Buckman-style add-on acquisitions** — From 2026-04-13 Wisory Team Check-In #wisory_product ^task-20260413-003
- [ ] **Locate existing culture/team/leadership indices slides and send to Jason Whitney for XTI appendix** — From 2026-04-13 Wisory Team Check-In #wisory_product ^task-20260413-004
- [ ] **Draft concise high-level Intel IQ platform description (software + AI + tech stack + differentiation) for investors/prospects/analysts** — From 2026-04-13 Wisory Team Check-In #wisory_product ^task-20260413-005
- [ ] **Research and outline Intel IQ security overview (data room access, storage, encryption, API, protection) for prospect Q&A** — From 2026-04-13 Wisory Team Check-In #wisory_product ^task-20260413-006
- [ ] **Send revised shares + advisor agreements back to Elmer (copy Jason) — starts the 30-day equity clock** — From 2026-04-13 Regis/Jason 1:1 #wisory_product ^task-20260413-007
- [ ] **Email Jason the Intel IQ origin-story document for review** — From 2026-04-13 Regis/Jason 1:1 #wisory_product ^task-20260413-008
- [ ] **Outline Intel IQ tooling changes needed to support IU Ventures + broader university engagement model** — From 2026-04-13 Regis/Jason 1:1 #wisory_product ^task-20260413-009
- [ ] **Attend next advisory group call for latest Intel IQ updates** — From 2026-04-13 Regis/Jason 1:1 #wisory_product ^task-20260413-010
- [ ] **Sort Chicago trip dates** — Confirm travel dates and logistics #family ^task-20260414-001
- [ ] **Complete Wokelo analysis** #wisory_product ^task-20260414-002
- [ ] **Start Python best practices audit** — IntelliQ codebase review #wisory_product ^task-20260414-003
- [ ] **Draft first Conrad marketing frameworks (positioning, messaging) by Monday ahead of Leo's Tuesday Zillow meeting** — From 2026-04-09 Conrad Consumer Portal Strategy #job_search ^task-20260413-011
- [ ] **Share drafted Conrad marketing frameworks with Conrad team once ready** — From 2026-04-09 Conrad Consumer Portal Strategy #job_search ^task-20260413-012
- [ ] **Review research article plan** #thought_leadership ^task-20260405-015
- [x] **Set up meeting with Leo** — Conrad follow-up #wisory_product ^task-20260405-007 ✅ 2026-04-07
- [ ] **Rerun brand framework based on team feedback** #wisory_product ^task-20260405-008
- [ ] **Determine business expenses in AMEX account and pay out of Found** #family ^task-20260405-009
- [ ] **Pay balance of AMEX account out of North Shore bank** #family ^task-20260405-010
- [x] **Share new triathlon plan with Dex and integrate into Google Calendar** #family ^task-20260405-011 ✅ 2026-04-07
- [x] **Add Wisory Score Claude skill to Claude Code** #wisory_product ^task-20260405-012 ✅ 2026-04-07
- [ ] **Set up call with Beth Brockland** #wisory_product ^task-20260405-013
- [b] **Review Veritonic Insights product after Scott's update** — Waiting on Scott #job_search ^task-20260310-001
- [x] **Review Conrad deck, Splittero and Fund Loans materials** — Waiting on Dan Sogorka to send materials #job_search ^task-20260312-002 ✅ 2026-03-28
- [x] **Perform diagnostic scoring of Wisory's current public-facing materials** — Start of engagement, use frameworks #wisory_product ^task-20260316-006 ✅ 2026-03-28
- [x] **Send frameworks and initial slide examples to Conrad team for iteration** — Revised based on Leo's feedback #job_search ^task-20260316-008 ✅ 2026-03-18
- [b] **Run clarity score/index on HubSpot Small Business Unit as demo for Don Moore** — Waiting on Don #job_search ^task-20260320-002
- [x] **Notify Marcy at Barr Engineering if status or timelines change on other offers** — ~30 day window on verbals. Marcy following up next week. #job_search ^task-20260320-003 ✅ 2026-04-07

## Recurring (Monthly)

- [ ] **Review LangGraph releases for upgrades** — https://github.com/langchain-ai/langgraph/releases — Check 1st of each month #wisory_product ^task-recurring-langgraph

## P3 - Backlog

- [ ] **Develop referral/finder-fee compensation structure for CTV advisors** — Commission-based model for opening CTV doors, share with Scott #job_search ^task-20260310-002
- [ ] **Document industry-vs-company scoring consideration for Wisory Score model** — Harriette raised how industry context affects company scoring — not yet clear if needed #wisory_product ^task-20260311-004

## Completed

- [x] **Identify preferred starting topic (AI use case or FinTech) for Dr. Streff follow-up** — Baker's Institute #thought_leadership ^task-20260309-001 ✅ 2026-03-14
- [x] **Send Outdoor recruiting tool materials to Jim Black** — Fog Line Advisors #job_search ^task-20260310-003 ✅ 2026-03-14
- [x] **Send slide deck to Jason W for review, then forward to Sean (with NDA)** — The Wisory #wisory_product ^task-20260311-001 ✅ 2026-03-14
- [x] **Send slide deck and supporting slides to all participants** — Wisory Score 2.0 / Sean intro #wisory_product ^task-20260311-003 ✅ 2026-03-14
- [x] **Send pre-call notes about Conrad to Adam and Leo** — Conrad intro #job_search ^task-20260313-001 ✅ 2026-03-14
- [x] **Talk with Dan Sogorka about Conrad's current work and identify investor intros** — Conrad #job_search ^task-20260313-002 ✅ 2026-03-14
- [x] **Reply to LinkedIn messages** — Inbox zero on DMs #job_search ^task-20260316-001 ✅ 2026-03-16
- [x] **Publish LinkedIn newsletter** — Weekly newsletter published #thought_leadership ^task-20260316-002 ✅ 2026-03-16
- [x] **Write and publish Substack article** — Weekly Substack published #thought_leadership ^task-20260320-001 ✅ 2026-03-20
- [x] **Send Don Moore the project deck, 66-component list, and related files via shared Google Drive** — Materials shared for April/May outreach blitz #job_search ^task-20260320-001b ✅ 2026-03-20
- [x] **Develop new slides showing how Wisory tool works and differentiates from off-the-shelf AI** — Jason confirmed he has what he needs #wisory_product ^task-20260311-002 ✅ 2026-03-20
- [x] **Schedule recurring weekly check-in with Harriet** — Recurring meeting set #wisory_product ^task-20260316-005 ✅ 2026-03-20
- [x] **Follow up with Kwasi Dwomoh on Experian interview status** — HM interview scheduled for Thu Mar 26 #job_search ^task-20260318-002 ✅ 2026-03-20

---

## Task Format

```
- [ ] **Task title** — Context or notes #pillar
- [s] Started task
- [b] Blocked task (note blocker)
- [x] Completed task
```

## Pillars

Tasks should align to your strategic pillars (configured during `/setup`).
