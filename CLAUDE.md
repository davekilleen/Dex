...
- Adaptive based on available data (calendar, OpenOats/Gladia transcripts, or neither)
...
### Frontmatter Schemas (Mandatory for People, Companies, Projects)

When creating or updating any page in `05-Areas/People/`, `05-Areas/Companies/`, or
`04-Projects/`, include the appropriate YAML frontmatter block at the very top of
the file. Fields not yet known should be left empty (`key: `) rather than omitted —
this keeps Bases columns consistent across the vault.
...
Rules:
- Preserve any existing frontmatter keys not listed above — don't strip them.
- When updating an existing page (e.g. logging a new meeting), refresh `last_met`
  to the new meeting date.
- Don't auto-fill `company`, `role`, `linkedin`, `domain`, `stage` unless the
  source material gives you a clear value — leave empty rather than guess.

### Obsidian Graph Coloring

Folder-based color groups in `.obsidian/graph.json` are generated by
`.scripts/update-obsidian-graph.cjs`. Palette is at the top of that file.

When to regenerate:
- After adding a new top-level folder you want to color.
- After changing the palette.
- If `graph.json` gets reset (Obsidian sometimes does this on fresh install).

Commands:
- Preview: `node .scripts/update-obsidian-graph.cjs --dry-run`
- Apply:   `node .scripts/update-obsidian-graph.cjs`

Sub-folders are listed before parents in the script — Obsidian applies the
first matching `path:` query, so the more-specific one wins.

Optional CSS defaults at `.obsidian/snippets/graph-colors.css` (toggle on under
...
Meetings are recorded and transcribed through two tiers:

1. **OpenOats (on-device, immediate):** Runs locally during meetings using parakeetV2 or whisperLargeV3Turbo. Low quality but zero-latency. Recordings and transcripts land in `/Users/davidorban/Documents/OpenOats/`.

2. **Gladia (cloud, high quality):** Diarized transcription via Gladia API. ~50-65% readability improvement over OpenOats. Run via batch script in `/Users/davidorban/Dev/transcription-workflow/`. API key in 1Password ("API Credentials Gladia").

**When both exist:** Always prefer Gladia transcripts for processing.

**To transcribe recordings:** Use the batch script at `/Users/davidorban/Documents/OpenOats/batch_transcribe_today.py` with the transcription-workflow venv.
...
6. **Generate meeting cache entry:** After creating/updating the meeting note, run:
   ```bash
   node .scripts/meeting-cache-generator.cjs 00-Inbox/Meetings/<filename>.md
   ```
   This creates a compact YAML summary in `06-Resources/Meeting_Cache/` and updates the person-meeting index.

**Meeting Cache (Primary Lookup):** When looking up meeting context (for `/meeting-prep`, person page enrichment, weekly reviews, or any meeting-related query), read the YAML cache in `06-Resources/Meeting_Cache/` and the index at `06-Resources/Meeting_Cache/_index.json` FIRST. Only open full meeting notes in `00-Inbox/Meetings/` when the user asks for specific details or the cache entry lacks the needed information. The cache contains: summary, decisions, action_items, key_points, topics, people_mentioned, and a pointer to the full note.
...
- **From meeting transcripts**: Extract feedback and development discussions from manager 1:1s
...
- Meeting notes: `YYYY-MM-DD-Meeting-Topic.md` (no spaces — use dashes between all words)
