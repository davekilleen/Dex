...
<!-- Generated from `.claude/skills/getting-started/SKILL.md` by `scripts/generate-agents-skills.py`. Do not edit. -->
...
Use prompt_user tool. If prompt_user is not available, prompt in CLI with the same numbered options and capture the selection:
```json
{
  "questions": [{
    "id": "granola_strategy",
...
def map_strategy_to_ranges(strategy: str, extent: dict) -> dict:
    ...
    if strategy == "smart":
        ...
        return None
    # For "custom", ranges come from separate prompt_user responses (or CLI fallback)
```
...
Use the same prompt_user and processing logic as Flow A (or CLI fallback).
...
```bash
Use the harness's MCP integration flow if available; otherwise continue without optional integrations.
```
...
Call the `mark_feature_used` tool on the `dex-analytics` MCP server with `getting-started`. This ticks the feature's box in `System/usage_log.md`, which is what `/dex-level-up` reads to recommend features that have not been tried. It writes locally and sends nothing, so it runs whether or not analytics is enabled.
