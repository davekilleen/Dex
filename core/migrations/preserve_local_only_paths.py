...
def preview(repo: Path, policy_path: Path | None = None) -> dict[str, Any]:
    ...
    if state == "blocked-query-mismatch":
        ...
        # the user with a bare count mismatch (issue #242 item 4).
        missing = sorted(policy_paths - actual)
        unexpected = sorted(actual - policy_paths)
        result["missing_from_baseline"] = missing[:50]
        result["missing_never_present"] = [
            relative for relative in missing if not (repo / relative).exists()
        ][:50]
        result["unexpected_tracked_ignored"] = unexpected[:50]
        result["guidance"] = (
