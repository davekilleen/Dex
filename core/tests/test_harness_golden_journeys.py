...
def test_code_editors_have_how_to_leave_lines() -> None:
    guide = (REPO_ROOT / "docs" / "HARNESS-PORTABILITY.md").read_text(encoding="utf-8")
    leave_rows = {row[0]: row[1] for row in _markdown_table_rows(guide, _LEAVE_TABLE_HEADER)}

    assert "GitHub Copilot CLI" in leave_rows
    assert "Visual Studio Code" in leave_rows
    family = leave_rows["GitHub Copilot CLI"]
    editor = leave_rows["Visual Studio Code"]
    assert "copilot plugin uninstall" in family.lower()
    assert "~/.copilot/installed-plugins/_direct/" in family
    assert "leftover" in family.lower()
    assert "chat.plugins.enabled" in editor
    assert "chat.pluginLocations" in editor
    assert "leftover" in editor.lower()
    assert "this runner will not invent that grant" not in editor.lower()
