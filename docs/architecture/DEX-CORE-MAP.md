...
**Governing principle.** "Hard on Core, gentle on user skills": Core-shipped skills get held to the trigger/quality bar and can be consolidated/rewritten; user-authored skills live in the `.claude/skills-custom/` vault-class directory and are protected from updates and left alone. `create-skill` writes them there. A `-custom` folder name is only for a user copy whose bare name would collide with a shipped skill, and for the keep-both sidecar under `.claude/skills/`.
