...
| Connection Manager (OAuth/token) | **SHIPPED, PRODUCT-INERT** | `core/integrations/connection-manager/` | Local-first OAuth via Nango catalog-as-data; encrypted on-device tokens; Doctor/contract plumbing exists, but no user-facing `/connect` doorway ships |
...
**Status.** The original engine passed its first live-account gate on 2026-07-24 (runbook: `docs/solutions/connection-manager-live-account-gate.md`) and ships in v1.73. Phase 2 then lifted Desktop's judgment layer into `lib/` (pinned at dex-desktop `2b34aa4d`, hash-verified by `lifted-conformance.test.cjs`): one refresher, bounded Google/Slack/Linear probes, a durable evidence ledger, `status --json`, and Doctor reading real engine health. Phase 3 froze the Desktop consumer contract and engine manifest. **PRODUCT-INERT:** no user-facing `/connect` doorway or session-start connection hook ships. The post-Phase-2/3 Google + Linear live-account rerun remains the final manual gate, and `/connect` must still be implemented and tested as a complete doorway before it can be claimed. Licence note: Nango providers is Elastic License 2.0 (source-available), consumed as a pinned npm dependency, not vendored; never re-expose the catalog as a managed service.
...
2. **A whole local-first OAuth stack (Connection Manager) ships but has no product doorway** — Nango-catalog-as-data, PKCE, encrypted on-device tokens, a health/refresh state machine, Doctor status, and a frozen Desktop contract. The original engine passed live Google/Linear checks; the post-Phase-2/3 rerun and a complete tested `/connect` doorway still gate any user-facing claim.
...
- **Connection Manager gate status — KNOWN.** The #209 engine passed Google and Linear live-account checks before v1.73. Phase 2/3 changed the health/contract layer afterward, so a Google + Linear rerun remains mandatory. No shipped `/connect` doorway currently depends on it.
