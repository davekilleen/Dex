...
| Visual Studio Code | Turn off `chat.plugins.enabled` or remove `./packages/dex-agent-plugin` from `chat.pluginLocations`, then reload. Leftover: those settings in this editor. |
| Kiro | Remove the custom Dex power from the Powers panel. Leftover: none that Dex owns; Dex does not claim a cache here. |
