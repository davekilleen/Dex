...
| Visual Studio Code | Local plugin settings stay off by default; the person points `chat.pluginLocations` at the shipped package and confirms the host in `/setup` when auto-detect is wrong |
| Kiro | Local custom-power import of the shipped package; Kiro wakes Dex when named; confirm Kiro in `/setup` when auto-detect is wrong |
...
| Copilot CLI | Root manifest follows the Open Plugin Spec accepted by Copilot; package and schema tests pass. Its incompatible hook schema is now explicitly unavailable rather than overclaimed. The local install command, list check, and Dex-folder start are now named. | Ubuntu Cloud is not a person opening this terminal. Live `copilot plugin install` remains unrecorded. |
| Visual Studio Code | The same open package is named. Plugin settings stay off by default. The person turns on `chat.plugins.enabled`, points `chat.pluginLocations` at `./packages/dex-agent-plugin`, and uses `/setup` if auto-detect is unreliable. | Ubuntu Cloud is not a person opening VS Code. Live enablement remains unrecorded. Hooks are not claimed. |
| Kiro | The same open package is named. Kiro imports it as a custom power and wakes Dex when named because `plugin.json` already lists `dex`. `/setup` is the fallback when auto-detect is unreliable. | Ubuntu Cloud is not a person opening Kiro. Live import remains unrecorded. Hooks are not claimed. |
| Pi | Native `dex-pi/extensions/dex/package.json` and lifecycle extension are pinned to clean `dex-pi` commit `5bf33ad7b23a06a890b25445cb1b4f4077b2ac19`; Core verifies the manifest hash and lifecycle source markers when that checkout is available. | No Pi package was changed or released; live installation remains a release-candidate check. |
| Agent Plugin | v1 schema, Node-to-Python launcher, tools, resources, dependency closure, and Linux deferred-runtime journey pass golden tests. A mandatory macOS/Windows CI matrix is wired. Stable and beta release jobs now build and attach the Claude Desktop MCPB, Gemini archive, and channel-labelled checksum index through the existing verified draft-first publisher. | Exact-commit macOS and Windows evidence belongs to the draft pull request and must be green on the final review head. No release has run, mandatory Fable reviews remain required, and client-specific hooks stay outside the v1 floor. |
