...
| rituals.duration | reads the ritual clock's log (`System/.dex/ritual-timings.jsonl`, written by `.claude/hooks/ritual-timing.py`) and judges each ritual's latest-week median on Dex's working time (the stretches between a prompt and the next hand-back; time waiting on the person is recorded, never judged) and on time-to-first-hand-back against its own previous four weeks; a run ends on its completion event when one arrives, otherwise inferred when the next skill starts or the session ends; every finished run carries the Dex version and the skill file's checksum, so a finding names the week the slowdown arrived, the releases on either side, and whether the skill is still the shipped one (Lens registry pin) or edited locally | — (OK with "no finished runs yet" until a ritual has handed back once; unreadable log → UNKNOWN) | a ritual with ≥3 runs this week and ≥3 in the baseline whose median is both ≥2 minutes and ≥1.5× its baseline, on working time or time to the first hand-back (T3: report with `/feedback`, which attaches the timing history; a locally edited skill is pointed at the edit first) |
...
| calendar.access | If `calendar.provider` is `google`, the Google Calendar `calendarList` path used by Google Workspace (not EventKit). A missing Google Workspace MCP registration or token is UNKNOWN, not BROKEN — planning may still use a session connector this checkup cannot see. Otherwise the `calendar_list_calendars` path via EventKit; configured `work_calendar` present in the list | provider is `none`, or Apple permission never granted AND feature unused | Google authorization failed with no refresh token, or 403 (T3: `/google-workspace-setup`) / Apple permission denied (T3) / configured calendar not found (T3: set `calendar.work_calendar`, list real names) |
