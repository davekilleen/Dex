...
| jobs.loaded | enumerates every `~/Library/LaunchAgents/*.plist`. Dex-named plists (a whole `dex` label segment, or legacy `com.claudesidian.*`) are always considered; other names count only on path evidence. Ownership is path evidence: an absolute `ProgramArguments` path under the checked vault claims the agent (any label, so user-installed `com.<user>.dex.*` jobs are covered); a plist whose strings reference the vault's stored former root (`~/.config/dex/vault-path` breadcrumb) is owned-and-stale → BROKEN with a T2 repoint-or-remove heal, never skipped as another product's. Owned agents get `launchctl list` state + interpreter exists+executable (ProgramArguments[0]). Same-label agents for another vault are skipped; an unreadable Dex-named plist is `UNKNOWN`, not assumed foreign. | no attributable plist installed | installed but unloaded (T2 load) / stale former-vault paths (T2 repoint/remove) / interpreter missing (T3 install node) |
| jobs.fresh | audits the health promise register (`core/health/promises.py`): each installed job's success receipt vs its promised cadence (meeting-intel `lastSync` >48 h, smoke-nightly success log >26 h, changelog-checker/learning-review activity logs >7 d). Receipts prove completion, not activity — a job that keeps running while failing reports BROKEN. Daemons and same-label foreign jobs are ignored; an unreadable plist is `UNKNOWN`. Owned agents with no registered promise (user-installed jobs) are named in the detail as "checked for loading only, not freshness" so OFF can't read as full coverage. | no attributable job installed | receipt missing ("never recorded a completed run") or stale beyond cadence (detail says last-success date) |
