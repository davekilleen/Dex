...
| jobs.loaded | classifies every `~/Library/LaunchAgents/*.plist` once per run (shared with jobs.fresh, via `core/utils/launch_agents.py` — the same module the session-start hook calls, so hook warnings and doctor findings can never diverge). Ownership is path evidence, never the filename: a `ProgramArguments` invocation referencing the checked vault (absolute path or embedded in a shell string) claims the agent under any label, so user-installed jobs are covered; a plist whose strings reference the vault's stored former root (`~/.config/dex/vault-path` breadcrumb, boundary-matched) is owned-and-stale → BROKEN with a T2 repoint-or-remove heal, never skipped as another product's — including owned agents with leftover former-root keys (partially repointed). The shipped-name heuristic (`com.dex.` / `com.claudesidian.` prefixes only, so `com.samsung.dex.*` / `com.dexcom.*` are never touched) exists solely so a corrupt shipped plist reads `UNKNOWN` instead of foreign; a corrupt third-party plist can never degrade the probe. Owned agents get `launchctl list` state + interpreter exists+executable (ProgramArguments[0]). Same-label agents for another vault are skipped. | no attributable plist installed | installed but unloaded (T2 load) / stale former-vault paths (T2 repoint/remove) / interpreter missing (T3 install node) |
