...
## [1.19.0] — Office 365 Calendar, Notion Sync, and Slack Bot (2026-03-14)

Three new integrations that make Dex work beyond macOS-only setups and accessible from your phone.
...
Weekly priorities generated by `/week-plan` now automatically sync to a Notion database page.

**What changed:**
* Automatic sync via macOS LaunchAgent (every 30 minutes)
* Deduplicates by archiving previous versions
* Install with `bash .scripts/install-week-priority-sync.sh`
* Requires `NOTION_API_TOKEN` and `NOTION_WEEK_DB_ID` in `.env`

### Slack Conversational Interface (Mobile Dex)

A full chief-of-staff bot you can DM from your phone — day overviews, calendar queries, person lookups, task creation, and meeting prep.

**What changed:**
* Two-pass LLM architecture: classify intent, fetch data, generate response
* SQLite-backed conversation memory with context windowing
* Proactive intelligence: EOD check-ins, morning briefs, stale priority alerts
* Work MCP bridge for task and calendar access from Slack
* Socket Mode — no public HTTP endpoint required

### Security Hardening

* Fixed command injection vulnerability in vault search (switched to `execFileSync`)
* Removed hardcoded Notion resource IDs from source code
* Reduced OAuth scope from `Calendars.ReadWrite` to `Calendars.Read` (least privilege)
* Added `.gitignore` rules for bot state files, WhatsApp config, and integration credentials
* Removed accidentally tracked state files from git history
* Added `.env.example` documenting all required environment variables
* Restricted `.env` file permissions to owner-only (600)
