---
name: daily-plan
description: Build today's plan from calendar, tasks, priorities and commitments,
  with smart scheduling suggestions. Use when the user says 'plan my day', 'what's
  on today', 'help me focus', or starts the morning. Also use proactively at the first
  session of the day. Not for reviewing a finished day; use `daily-review`.
---
## Execution mode
...
So the gathering phase is delegated to one subagent via the Task tool, using the self-contained prompt in this skill's
`AGENT_INSTRUCTIONS.md`:

1. Read `.opencode/skill/daily-plan/AGENT_INSTRUCTIONS.md`.
