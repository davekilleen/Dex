---
name: pipeline-sync
description: Live view of your Pipedrive pipeline reconciled against your local pipeline
  tracker; flags drift, maps focus deals, and pushes confirmed updates to the CRM.
  Use when the user says 'sync my pipeline', 'show me my pipeline', 'reconcile the
  CRM'. Not for connecting Pipedrive in the first place; use `pipedrive-setup`.
---
## Purpose
...
## Step 1: Delegate the gather (subagent task)

Spawn a `general` subagent via the Task tool, passing as its prompt the contents of `.opencode/skill/pipeline-sync/AGENT_INSTRUCTIONS.md` (substitute the mode: reconcile | map | discover). The subagent does the heavy read/diff work in its own context and returns a compact structured report:
...
If the subagent fails, fall back to running the instructions inline.
