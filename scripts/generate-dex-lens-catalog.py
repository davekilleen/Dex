...
def _host_adapters_for_skill(
    ...
    *,
    ...
    for profile in profiles:
        ...
        raw_lens_id = adapter.get("lens_adapter_id", profile["id"]) if isinstance(adapter, Mapping) else profile["id"]
        lens_id = _catalog_id_tuple(
            [raw_lens_id],
            context=f"{HARNESS_REGISTRY_PATH} profile {profile['id']} Lens adapter ids",
        )[0]
        capable.append(lens_id)
        if isinstance(adapter, Mapping) and adapter.get("kind") == "claude-plugin":
            claude_native.append(lens_id)
    if len(capable) != len(set(capable)):
        raise LensCatalogError(f"{HARNESS_REGISTRY_PATH} resolves duplicate Lens adapter ids")
