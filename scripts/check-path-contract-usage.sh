...
# core/utils/smoke.py is the smoke-test harness; its fixtures intentionally
# construct literal vault layouts under temporary directories.
ALLOWLIST='^(core/paths\.py|core/utils/smoke\.py|\.claude/hooks/paths\.cjs|\.claude/hooks/(company-context-injector|person-context-injector)\.cjs|scripts/check-path-consistency\.sh|scripts/verify-distribution\.sh|scripts/check-path-contract-usage\.sh|core/migrations/|.*\.sh$|\.claude/skills/[a-z-]+/scripts/)'
