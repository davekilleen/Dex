...
# core/runtime/paths.cjs is the neutral CJS contract; the Claude path is a thin
# wrapper. The packaged core/paths.py is byte-gated against its canonical source
# by generate-portable-plugin.py and the portable-plugin tests.
...
ALLOWLIST='^(core/paths\.py|core/runtime/paths\.cjs|packages/dex-agent-plugin/runtime/core/paths\.py|packages/dex-obsidian-plugin/paths\.js|core/portable_contract\.py|core/lifecycle/inventory\.py|core/tests/test_portable_contract\.py|\.claude/hooks/paths\.cjs|\.claude/hooks/(company-context-injector|person-context-injector)\.cjs|scripts/check-path-consistency\.sh|scripts/verify-distribution\.sh|scripts/check-path-contract-usage\.sh|core/migrations/|.*\.sh$|\.claude/skills/[a-z-]+/scripts/)'
