...
live install and does not invent that grant. No page includes a publish step.
...
def expected_sight(sentence: str, *, stop: bool) -> str:
    ...
    if "run `" in lower or "run `" in sentence.lower() or sentence.strip().startswith("From "):
        return (
...
def render_index(adapters: list[dict]) -> str:
    ...
    for adapter in adapters:
        harness_id = adapter["harness_id"]
        stop = " — stops at the folder grant" if harness_id == WORK_ID else ""
        lines.append(f"- [`{harness_id}`](./{harness_id}.md){stop}")
    lines.extend(
        [
            "",
            "## How to regenerate",
