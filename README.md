...
Cursor, ChatGPT, and Codex should call Dex's `boot_today` tool at session start (quarter goals, week priorities, pillars, urgent tasks) and `get_person_context` when a person is mentioned. Claude Code still auto-fires those same facts via hooks. The inventory of scheduled vs inject vs gates is in [`docs/architecture/HOOK-INVENTORY.md`](docs/architecture/HOOK-INVENTORY.md).
