...
description: "The PR-opening mechanics invoked at the end of every other playbook — worktrees, commit shaping, titles, description sections, stack sizing, readiness; load whenever finished work is about to become a PR."
---
...
**Titles.** Use Conventional Commits in the form `type(scope): subject`. Use `feat`, `fix`, `docs`, `refactor`, `test`, `chore`, or `perf` as the type. Use the changed area, such as `core` or `hooks`, as the scope. Keep the subject short and imperative. Apply the same `/technical-writing` and `/unslop` pass as the body. Name a real symbol when one carries the change. For example, `fix(core): retarget opening-a-pr babysit trigger`. Do not add a trailing period.
...
**Size and stacks.** Prefer five narrow PRs to one large PR. Stack follow-ups with Graphite (`gt`), and keep the ordered stack visible to reviewers. Branch from main only for independent work. Rebase on `main` before substantial stack work. [If this repo does not use Graphite stacks, the plain git/gh equivalent applies; keep the step's intent.]
