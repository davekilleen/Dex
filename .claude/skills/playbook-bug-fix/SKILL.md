...
description: "Owning a bug fix end to end — reproduce it yourself, binary-search the cause with runtime evidence, ship the smallest justified fix, verify on the same surface; load when the task is fixing broken behavior."
---
...
3. Plan the fix. If it crosses a function boundary, `architect` first. Delegate implementation to a subagent using your configured bug-fix model (default: a judgment model) with a specific scope; review the diff.
