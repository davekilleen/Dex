---
name: playbook-hillclimb
description: "Sustained iterative improvement of one measurable metric against a target — one change, one measurement, keep or revert, with a frozen harness and a decision log; load for 'hillclimb on X', 'make <metric> N% better', 'keep trying until it improves'."
---
...
5. Loop, one hypothesis per iteration:
   ...
   Each iteration ends in a check before the next begins (the **sequence-verifiable-units** principle skill). If the run is unattended, borrow only the wake mechanism from the Autonomous run playbook (playbook-autonomous-run), not its stop rule. This playbook's stop criteria below govern, so a plateau means pivot, not stop.
