---
name: playbook-orchestrate
description: "Running a multi-day program of many stacked PRs through dozens to hundreds of subagents from one standing coordinator chat; load only when the work outlives any single agent — one task driven to a predicate is playbook-autonomous-run instead."
---
...
Create `orchestrate/<project-slug>/` in the current agent's store (path in the system prompt). Every file has exactly one writer; owners publish facts, readers aggregate at read time. The canonical plain TSV and JSON stay readable without any CLI.
...
A sub-coordinator brief adds its track boundary and unit list, its spawn budget with the background default and the foreground exception list, the drain protocol, and the rollup format (per child: name, status, PR, head SHA, verdict, one line; plus track status and frontier delta).
...
2. **Install the runtime.** Create the store. Open the trail via the show-me-your-work skill, write the standing orders before any spawn, and seed `frontier.json` from existing PRs.
...
- Drain in batches at four points: the end of a critical section, a track rollup, a frontier watcher wake (arm it via the loop skill, with a long heartbeat fallback), and before a human report. Begin each batch by draining the inbox. Arrivals during a drain wait for the next one.
...
- A drain turn ends with three status lines: counts against the states, what changed, gates open. Detail lives in `status.md`; the full reply contract applies at checkpoints and close.

#### Stack safety

- The frontier is a computed object, never narrative. Recompute `frontier.json` from `gt` after every merge and stack mutation because GitHub base refs drift mid-restack while gt tracking is authoritative: ordered PR list, branch names, head SHAs, a generation number, the lowest unmerged PR. Resolve it where gt knows the stack, normally the stacker's clone; a checkout whose gt metadata never saw the submits reports no PRs and the command errors rather than guessing. [If this repo does not use Graphite stacks, the plain git/gh equivalent applies; keep the step's intent.]
- Exactly one stacker per stack may run `gt`, serialized within its stack; record the holder in the standing orders. Restacks run in a background subagent; a foreground restack at this scale takes the session down.
- Workers never rebase and never run `gt`. Babysitters follow playbook-babysit, one per stack, scoped to one immutable frontier generation; they report conflicts to the stacker rather than restacking.
...
Write ledger rows into `ledger.tsv`, one row per verdict, keyed by PR number plus head SHA: `live-ui-verified | unit-test-verified | type-check-only | verifier-blocked | verifier-failed`. CI green is an input to a verdict, not a verdict. Behavioral work needs better than `type-check-only`. `verifier-blocked` is not a pass; respawn when the environment heals. `verifier-failed` gets a fix unit, not a re-verify. A worker may self-report; a verifier overrides it on the same key. A new head SHA voids the row, so re-verify after restack. The ledger answers "was this verified", not memory and not the transcript.

A unit is not done until its output is externalized the moment it lands, never batched to the end of the run: a worker pushes its branch, a verifier writes its ledger row, receipts land in the store. Work that exists only in one agent's session when that session dies was never done.

#### Liveness and failure

- Never resume an agent to check on it; a resume restarts an idle agent. Probe read-only: the ledger, `units.tsv`, `gh`, pushed branches, the background agent's status in the harness. Transcript mtime is not liveness.
...
- After a harness restart: local agents are dead, pushed and recorded work is not. Re-read the standing orders and `units.tsv`, recompute the frontier, reattach remote work by PR and branch rather than agent id, respawn one sub-coordinator per track from its stored brief plus current state, drain, resume. The dead session's store lock clears itself on the next write; replace a lock whose holder pid is gone.
