...
This skill is vault-aware. Before falling back to asking the user for information, check the following Dex locations:

**During `kickoff` — seed the storybank from the vault:**
- Read `05-Areas/Career/[Name]_Profile.md` if it exists — extract proof points, metrics, and role history to pre-populate story seeds rather than starting from scratch
- Read `05-Areas/Career/Evidence/` if it exists — any captured achievements can seed the storybank directly
- Read `01-Quarter_Goals/Quarter_Goals.md` if it exists — current goals reveal what skills the user is actively developing

**During `research` and `prep [company]` — check existing vault context:**
- Check `05-Areas/Opportunities/` for an existing opportunity file for the company — it may already contain role details, fit assessment, and questions prepared
- Check `05-Areas/People/External/` for person pages on interviewers — pull role, relationship notes, and meeting history rather than asking the user to describe them
- Check `05-Areas/Companies/` for any existing company context

**During `stories` and `analyze` — use semantic search:**
- If the QMD `query` tool is available, use it to search for relevant achievements by meaning (e.g. "commercial impact", "team leadership", "technical credibility") — this finds evidence across meeting notes, projects, and career files that keyword search misses
- Fall back to grep if QMD is unavailable

**Session state:**
- `coaching_state.md` lives in the skill directory (`.claude/skills/interview-coach/coaching_state.md`) — this is the user's persistent coaching record. Never delete it during updates.
