...
description: "Landing a green stack safely — verify each PR independently, land only the verified run, arm the merge queue, watch the drain without driving it; load for 'land the stack', 'ship it', 'enable merge when ready'."
---
...
This is the half after playbook-babysit. Babysit makes a stack mergeable. Shipping decides what is actually safe to merge and lets Graphite drain it. Green is not safe, and the gap between those two words is where this playbook lives.

1. **Verify every PR independently before arming anything.** One subagent per PR, not batched, each a background subagent (`run_in_background: true`), each exercising the real surface (the project's UI or CLI verification skill, as the change demands) against parent versus head. Each returns `PASS`, `PASS+NOTES` or `FAIL` and posts that verdict on its own PR so the record outlives the chat. Safe means a verdict from an agent that did not write the code. CI green is not a verdict, and an approving bot review is not a verdict.
...
4. **Arm merge-when-ready through Graphite, and pass `--always`.** A no-op submit skips the Graphite update and silently arms nothing, which reads exactly like success. [If this repo does not use Graphite stacks, the plain git/gh equivalent applies; keep the step's intent.]
   ```bash
